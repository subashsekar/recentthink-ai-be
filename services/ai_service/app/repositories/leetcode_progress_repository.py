"""LeetCode progress data-access repository."""
...
class LeetCodeProgressRepository:
    """Repository for :class:`LeetCodeProgress` persistence."""

    def __init__(self, db: Session) -> None:
        self._db = db

    def get_or_create(self, user_id: UUID) -> LeetCodeProgress:
        progress = self.get_by_user_id(user_id)
        if progress is not None:
            return progress
        progress = LeetCodeProgress(user_id=user_id, weak_topics=[], strong_topics=[])
        try:
    ...
    def get_by_user_id(self, user_id: UUID) -> LeetCodeProgress | None:
        stmt = select(LeetCodeProgress).where(LeetCodeProgress.user_id == user_id)
